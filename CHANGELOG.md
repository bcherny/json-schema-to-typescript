...
This is a major release because it requires Node.js 22.19 or later (#842) and because the types emitted for schemas that already compiled change by default, and in one case the exported names do: a `$ref` that carries a `description` no longer declares a numbered copy of its target (#803), so `Foo1`/`Foo2` duplicates disappear and any that remain are renumbered. None of the output changes below has an option that restores the 16.0.0 output; pin `^16` until you have re-checked imports of generated names. Everything else is additive: six new options, `--imports`/`compileFiles()`, wider input types, a CLI that honours your Prettier configuration, fixes for crashes and the CLI, and a faster compiler.

Breaking — changes to emitted types or exported names for existing schemas:

- 32bdb81 Breaking: Node.js 22.19 or later is required (`engines.node >=22.19.0`; Node 16, 18 and 20 are end-of-life — stay on 16.x there), following `@apidevtools/json-schema-ref-parser`, upgraded from 11.9 to 16.0. With it: a `$ref` with sibling keywords always lays the siblings over the referenced schema, and merges a sibling keyword with the referenced schema's when both are objects (both `properties` maps apply) instead of replacing it (for siblings that extend the referenced schema, #840 below composes an intersection instead), so a few generated types gain members or become nullable where the schema said so — `$id` keeps naming types and `$ref`s keep resolving relative to the file they are in; HTTP(S) `$ref`s to loopback, private-network and internal hosts (`localhost`, `127.*`, `10.*`, `192.168.*`, `*.internal`, `*.corp`, `*.local`, …) or internal ports (8080, 8443, 5432, 6379, …) are refused by default and fail with `Unable to resolve $ref pointer "<url>"` — set `$refOptions: {resolve: {http: {safeUrlResolver: false}}}` to allow them again; a root `$ref` to a boolean schema declares the root type (`never` or `unknown`) instead of producing an empty module (#842)
- a6ffb64 Bugfix: A `$ref` with sibling annotations (`description`, `title`, `default`, `examples`, `deprecated`, `readOnly`, vendor keywords…) resolves to the referenced type instead of declaring a numbered copy of it (`Foo1`, `Foo2`…) or inlining it; the property carries its own `description`/`deprecated` as JSDoc, and a sibling `title` no longer names a type. Siblings that affect the type (`type`, `enum`, `properties`, `required`, `items`, `tsType`…) behave as before. Schemas that reference one definition from many documented properties emit far fewer declarations (the FHIR fixture: 5153 → 679), so exported names change: the numbered copies vanish and the numbering of any that remain shifts — code that imports `FooN` names has to be re-checked. Also: `default` next to `allOf` no longer adds `& string`/`& number` to the intersection, and a one-member `anyOf`/`allOf` around a schema that accepts any value keeps the member's name (#803, supersedes #683 by @LudaThomas; fixes #193, #334, #363, #472)
...
- 3ccce84 Bugfix: A member of a nested `anyOf`/`oneOf`/`allOf` that requires keys it doesn't declare borrows their types from an enclosing `anyOf`/`oneOf` member whose array `type` was narrowed to the parent's, as it already did when that member was written with the single type; previously those keys came out as `unknown` (#844 with #845, follow-up to #784 and #810)
- 8e65b2a, 754ffdc Bugfix: `unevaluatedProperties` (draft 2019-09+) is honoured like `additionalProperties` on plain objects (`false` closes the object, a schema types the index signature) and, for `false`, on ordinary `allOf`/`anyOf`/`oneOf` compositions; an explicit `additionalProperties` still wins. Objects with a schema-valued `unevaluatedProperties` next to `allOf`/`anyOf`/`oneOf`, or whose keys can also come from `if`/`then`/`else`, `dependentSchemas`, `$dynamicRef` or a `$ref` with sibling keywords, keep their open index signature, as in 16.0.0 (#782, re-host of #738 by @Neal006, and #798; fixes #442)
...
- de05d84 Bugfix: A root schema that only hosts `definitions`/`$defs` (no `type`, `properties` or other type-shaping keyword), or a root `$ref` to a definition with no type-shaping keyword, emits `export type Foo = unknown` instead of `export interface Foo {[k: string]: unknown}`; the definitions under it are declared as before (#839, follow-up to #806)
...
- e57fa95 Bugfix: OpenAPI `nullable: true` next to a `$ref` inside a file that is itself reached through a `$ref` compiles to `X | null` at the property, like it does in the root file; the referenced definition is no longer declared as `X = T | null` plus a numbered copy `X1 = T` for its non-nullable uses (#776 with #777; follow-up to #755)
...
- bdd1394 Bugfix: `properties`/`patternProperties` next to a non-object `type` on a root schema or a `$ref`'d definition are ignored, as they already were inline, instead of producing an impossible intersection (`{…} & string`) (#851)
- 84ae4d7 Bugfix: `extends` with a base that an interface cannot extend (empty, primitive, union, enum, closed-empty or untitled inline schema, or a base that is itself such a declaration) is printed as an intersection type instead of invalid TypeScript; bases that were legal to extend but are not plain interfaces (`allOf`, array, object `const`, object-typed `tsType`) switch to the intersection form too (#854)
- 1afd7fa Bugfix: `{}` as a `const`, as an `enum` member or nested in a `const` value is emitted as the closed empty object type `{[k: string]: never}` instead of the TypeScript type `{}`, which accepts any non-nullish value (#856)
- 0912dea Bugfix: A `tsType` or `formatTypes` type that is not a single type reference is parenthesized where it becomes an array element or an `anyOf`/`oneOf`/`allOf` member — `items: {tsType: '() => void'}` yields `(() => void)[]` (was `() => void[]`, a function returning an array), `keyof X` yields `(keyof X)[]`, `'A | B'` yields `(A | B)[]`, and `anyOf: [{tsType: '(x: number) => string'}, {type: 'null'}]` yields `((x: number) => string) | null`. Schemas without `tsType`/`formatTypes` are untouched (#852)
- 883b759 Bugfix: A carriage return (`\r` or `\r\n`) in a `description` is treated as a line break when its JSDoc comment is written, so the comment keeps its ` * ` prefix on every line and its indentation when formatted. Comments only; the types are unchanged (#853)
- e48166e Bugfix: The `description` of a `const` or inline `enum` member of a `oneOf`/`anyOf` is kept as a JSDoc comment above that union member instead of being dropped, as #773 already did for object members. Comments only; the types are unchanged (#869, supersedes #481; fixes #475)
- 5619ebc Bugfix: An `additionalProperties` schema on a schema without `properties` is kept next to `anyOf`/`oneOf`/`allOf`: it gives the schema its own object type, intersected with the members, wherever `properties` already did — and likewise next to a sibling `enum`/`const`, an untyped `items`, or a type inferred from `default`; previously it was dropped from the emitted type there (#858, follow-up to #851)
- dd33ded Bugfix: Enums declared through `tsEnumNames` or `inferStringEnumKeysFromValues` always compile: a member name TypeScript reads as a number (`"1"`, `"-1"`, `"2.5"`) gets a leading underscore (`_1 = "1"`; `"+1"`, `"01"`, `"1e3"` are legal and stay), an `enum` with a `null`, boolean, object or array value is typed as a union of its literals instead of an `enum` declaration TypeScript rejects, and a value listed twice becomes one member. A name listed twice in `tsEnumNames` is now a `ValidationError`, like a list of the wrong length — also where the names went unused and 16.0.0 emitted valid output (an `anyOf` branch, an unreferenced definition). Enums that compiled before are unchanged (#875)
- 956d822 Bugfix: A schema `title` equal to a built-in global type name (`Array`, `Map`, `Object`, `Promise`, `RegExp`, `Set`, `Symbol`) is emitted with a counter suffix (`Symbol1`) instead of declaring a type that shadows the global; consumers importing such a name have to re-check it (#874, fixes #386)
- 1e65392 Bugfix: A schema made only of `not`, `if`/`then`/`else` or `dependencies` (keywords this tool does not implement) emits `unknown`, like the empty schema, instead of `{[k: string]: unknown}`, and is left out of `allOf` intersections (#843, follow-up to #806)
- 9aacc4e Improvement: A `oneOf`/`anyOf`/`enum` member that is itself an anonymous union is printed as part of the enclosing union — `"a" | "b" | null` instead of `("a" | "b") | null`, `string | null | number` instead of `(string | null) | (number | null)`. Types accept the same values; only the spelling changes. A `const`/inline `enum` branch with its own `description` keeps its parentheses (#847)
- 3133f80 Breaking: A `$ref` with sibling keywords that extend the referenced schema — `properties`, `patternProperties`, `required`, `items`, `prefixItems`, `anyOf`, `allOf`, `oneOf` — is emitted as the referenced type intersected with the siblings (`export type Person = BaseType & {age?: number}`), as draft 2019-09+ defines and as the equivalent `allOf` already was, instead of a merged copy that dropped the referenced schema's own `properties`/`required` and could be declared under the referenced type's name. `title`/`description`/`default` next to such a `$ref` stay on the property or name the composite type; siblings that only restate or narrow the referenced type (`type`, `enum`, `const`, `additionalProperties`, `minItems`, `maxItems`, `format`) and `tsType` behave as before (#840, fixes #613)
- 3eb39bb Improvement: `unreachableDefinitions` (and `--imports`) no longer add a "This interface was referenced by `X`'s JSON-Schema via the `definition` "y"" comment to every declaration taken from `definitions`/`$defs`; the schema's own `description` is kept (#848, #428)
...
- d5766a4 Bugfix: A `$ref` to another file whose root is a chain of internal `$ref`s, or a pointer into a file whose root is itself a `$ref`, compiles instead of crashing with "Failed to resolve $ref" (#870, follow-up to #741)
...
- 7a11978 Bugfix: A `$ref` in a schema position that resolves to something that is not a schema — an empty file, a document that is just `null`, a string or a number, plain-text YAML, or a pointer onto a keyword's value — is rejected with a `ReferenceError` naming the `$ref`, where it sits, the file it resolved to and what was found there, instead of a `TypeError` in the parser or the value printed as a literal type (`y?: "just some text"`). Two inputs that compiled silently before are the same error now: a `definitions`/`$defs` entry that `$ref`s a prose or empty file, and such a `$ref` under `not`, `if`/`then`/`else` or a single `extends` (#877)
...
- e382f51 Bugfix (CLI): A usage error (extra arguments, a glob that matches nothing, one `.d.ts` output for a directory or glob input, an unsupported `--imports` combination) prints as one `error: …` line instead of a stack trace inside `[ ]`; other failures keep their stack, without the brackets. `--inferStringEnumKeysFromValues=false` and `--inferStringEnumKeysFromValues false` turn the option off instead of on, and the bare flag no longer takes the next argument as its value (#876)
...
- 5caacfc Improvement: `format: false` (`--no-format`) output is laid out readably as generated — indented by depth, with semicolons, parentheses only where needed, long unions one member per line and `const` objects as type literals; `format: true` output (the default) is unchanged. For very large schemas `--no-format` is now the recommended setting: two to three times faster in about half the memory (#846)
...
- c3321a5 Perf: `compile()` copies its input schema with a plain-data clone instead of lodash `cloneDeep`, which was quadratic in the number of schema nodes under bun: there, the AWS SAM schema (10 628 definitions) compiles in 1.7 s instead of 30.6 s and FHIR in 0.4 s instead of 0.8 s; under node (the CLI) the copy is slightly faster and nothing else moves. A caller's `Date`, `RegExp` or class instance inside the schema is still copied, a function still shared (#871)
...
- 32bdb81 The browserify bundle script (`npm run build:browser` → `dist/bundle.js`, last published in the 9.1.1 tarball, not built by CI or publish.yml) and its `browserify`/`browserify-shim`/`tsify` devDependencies are removed; bundle with your own ESM-aware bundler (#842)
- adc86c5, 26e1bd9, c984c3f Docs: README documents which JSON Schema drafts and post-draft-4 keywords are supported (#828, #120) and how to pass `$refOptions` to the CLI as dotted flags (#819, settles #632 by @ndopj; refs #631); `json2ts --help` says that with no output path results go to standard output (#863)
- 3a9affc CI compiles a corpus of real-world schemas under three option sets and type-checks the output with `tsc --strict`, and gates JSON-Schema-Test-Suite conformance against a recorded baseline (`bun run corpus`, `bun run conformance`; see test/corpus and test/conformance) (#861)
- 188cf89, 0d082c3, b59016f, 544e8d1 The repository's own tooling (install, scripts, tests, CI) moved from npm to bun; see CONTRIBUTING.md. The published package's file layout is unchanged (#762, #763). `bun run test` (and so CI) runs eslint first, over every file under `src/` (#865, #868); bug reports and feature requests open from issue forms (#862)
- 96dddc3 The repository builds and type-checks with TypeScript 7 (`tsc` 7.0); the 6.0 compiler API stays installed under the name `typescript` for eslint and the test harness. The published `.d.ts` files are byte-identical to the TypeScript 6 build and generated types are unchanged (#867)
- 2b03f25 `js-yaml` 5.2.3 → 5.4.1, the version a fresh install of 16.0.0 already resolved, so the repository now tests what users run. Differences from the 5.2.3 the lockfile pinned, none of which occur in this repository's schemas: a bare `<<` used as a YAML value is read as the string `"<<"` instead of being dropped, and a file that starts with a byte order mark parses (both as in 15.x); a merge key over a list of strings or numbers, a merge key over more than 100 mappings, or more than about 5 000 merges in one file is now an `Error parsing YML` (#866)
- Internal, no output change: root definitions are named once in the normalizer (#837); `applySchemaTyping` folded into `typesOfSchema` (#821, drops `dist/src/applySchemaTyping.*`); the unused `REFERENCE` AST kind and `maybeStripDefault` removed (#824, #797); one-use helpers inlined (#823, #795, #864); the root schema and owned `anyOf`/`oneOf` lists are told apart without reading `[Parent]` (#849)
- 088a993, e0de5eb, bddc041, 8f7f668 Tests: a slice of the seed fuzzer runs in CI (#830), covers the crash classes fixed since 16.0.0 and fails on a case that becomes very slow (#872); the output for empty `anyOf`/`oneOf`/`allOf` is pinned (#817, test from #634 by @k-yle); regression tests for a root `$ref` whose target contains a nested `$ref` (#805, from #723), empty `enum` arrays (#765) and `strictIndexSignatures` with a single `patternProperty` (#779)

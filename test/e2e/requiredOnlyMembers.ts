 ...
 *   isn't one of the parent's `properties`, so it is skipped, like everywhere else; a key listed
 *   twice (invalid, but tolerated everywhere else) is picked once.
 * - `oneOf` member 2: nothing it lists is declared, so there is nothing to pick and it parses on
 *   its own, as before (`{[k: string]: unknown}`).
 * - `oneOf` member 3: says something other than `required`, so it is parsed on its own too (and
 *   dedupes against member 2).
 */
export const input = {

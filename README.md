...
| format | boolean | `true` | Format the output with [Prettier](https://prettier.io/)? With `false` the output is still indented and readable, just not line-wrapped by Prettier, and large schemas compile two to three times faster in about half the memory (see [below](#json-schema-to-typescript-is-slow-or-runs-out-of-memory-on-my-giant-file-what-can-i-do)). |
...
| `$ref` with sibling keywords | supported | siblings that extend the referenced schema (`properties`, `patternProperties`, `required`, `items`, `allOf` / `anyOf` / `oneOf`) are intersected with it, as the equivalent `allOf` (`Foo & {...}`); annotations (`description`, `title`, `default`...) stay on the property and the reference resolves to `Foo` itself. Siblings that restate or adjust the referenced schema (`type` alone, `enum`, `const`, `additionalProperties`, `minItems` / `maxItems`, `format`) and `tsType` are still merged into a copy of it, which may be emitted as a second type (`Foo1`) | [#613](https://github.com/bcherny/json-schema-to-typescript/issues/613) |
...
### JSON-Schema-to-TypeScript is slow, or runs out of memory, on my giant file. What can I do?

Turn formatting off: `--no-format` on the command line, `format: false` in the API. Most of the time and much of the memory of compiling a large schema go to Prettier, and the unformatted output is indented, one member per line, and perfectly readable — it just isn't wrapped to a line width. Measured on this repository's benchmark set (`node bench/bench.mjs`, Node 22):

| schema | output | `format: true` | `format: false` |
|---|--:|--:|--:|
| Azure deployment template (60 linked schemas) | 8.6 MB | 8.3 s, 958 MB | 3.7 s, 415 MB |
| FHIR R4 | 2.3 MB | 0.89 s, 266 MB | 0.32 s, 164 MB |
| Kubernetes definitions (`unreachableDefinitions`) | 0.9 MB | 0.35 s, 155 MB | 0.11 s, 119 MB |

(wall-clock time of one `compile()` call and the peak memory of the process.) For anything you generate in a build step rather than commit and read, `format: false` is the recommended setting.

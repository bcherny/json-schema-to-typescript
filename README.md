...
| defaultAdditionalProperties | boolean | `true` | Default value for additionalProperties on `object` types that do not explicitly set it. Set this to false to generate more restrictive interfaces. Note that this is contrary to the spec, and should never be used to validate objects. |

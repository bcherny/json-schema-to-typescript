...
Resolves referenced schemas (in the file, on the local filesystem, or over the network). A document whose `$ref`s all point into itself is dereferenced in-process; anything else goes through [$RefParser](https://github.com/APIDevTools/json-schema-ref-parser), and the two give the same result (test/resolver.test.ts). Nodes that end up held in more than one place are marked (`Shared`), so that later rewrites meant for one place are made on a copy.
